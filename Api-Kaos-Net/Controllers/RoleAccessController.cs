...
namespace Api_Kaos_Net.Controllers
{
    ...
    public class RoleAccessController : ControllerBase
    {
        ...
        [HttpGet("{id}")]
        public async Task<ActionResult<RoleAccessDto>> GetById(int id)
        {
        ...
        [HttpPost]
        public async Task<ActionResult<RoleAccessDto>> Create(RoleAccessDto dto)
        {
            var created = await _service.CreateAsync(dto);
            return CreatedAtAction(nameof(GetById), new { id = created.RoleAccessId }, created);
        }

        [HttpPut("{id}")]
        public async Task<IActionResult> Update(int id, RoleAccessDto dto)
        {
